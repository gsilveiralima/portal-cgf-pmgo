...
const required = ['index.html','404.html','style.css','app.js','section.js','public-data.js','robots.txt','sitemap.xml','site.webmanifest','vercel.json','api/news.js','api/orientar.js','api/search.js','cgf-emblem.svg','cgf-hero-medallion.svg'];
for (const file of required) if (!fs.existsSync(path.join(root, file))) throw new Error(`Arquivo obrigatório ausente: ${file}`);
...
const vercel=JSON.parse(fs.readFileSync(path.join(root,'vercel.json'),'utf8'));const headers=JSON.stringify(vercel.headers||[]);for(const expected of ['Content-Security-Policy','X-Content-Type-Options','Permissions-Policy','Referrer-Policy'])if(!headers.includes(expected))throw new Error(`Header ausente: ${expected}`);
