...
const required = [
  ...
  'api/news.js','api/orientar.js','api/search.js',
...
for (const section of SECTIONS) {
  if (!fs.existsSync(path.join(root,'secoes',section.slug,'index.html'))) throw new Error(`Página ausente: ${section.slug}`);
}

const index = fs.readFileSync(path.join(root,'index.html'),'utf8');
const canonicalWorkRefs = [
  'cgf-emblem-digital.png',
  'cgf-hero-medallion.png',
  'assets/index-CzXOL65S.js',
  'assets/index-DfqFnF9R.css'
];
for (const ref of canonicalWorkRefs) {
...
if (index.includes('cgf-emblem.svg') || index.includes('cgf-hero-medallion.svg')) {
  throw new Error('A página principal voltou a usar identidade SVG substituta em vez dos PNGs aprovados no Work.');
}
for (const requiredMeta of ['rel="canonical"','rel="manifest"','og:url','twitter:card']) {
  if (!index.includes(requiredMeta)) throw new Error(`Metadado obrigatório ausente na página principal: ${requiredMeta}`);
}
...
for (const signature of ['.government-bar','.accessibility-controls','.hero__media','.finder-section','.quick-grid']) {
  if (!workCss.includes(signature)) throw new Error(`Assinatura visual do Work ausente no CSS: ${signature}`);
}
...
console.log(`Auditoria concluída: interface canônica do Work preservada, ${SECTIONS.length} seções, headers, sitemap e privacidade verificados.`);
